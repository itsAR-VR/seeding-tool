...
const afterDir = path.resolve(process.cwd(), "tests/screenshots/after");

async function capture(page: Parameters<typeof test>[1]["page"], name: string) {
  await fs.mkdir(afterDir, { recursive: true });
  await page.screenshot({ fullPage: true, path: path.join(afterDir, name) });
}

test.describe("visual after", () => {
