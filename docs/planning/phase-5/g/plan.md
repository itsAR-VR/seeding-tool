# Phase 5g — Capture campaign creation flow + expand platform catalog
...
## Terminus Maximus Execution Update — 2026-03-02
- Validated and fixed the core blocker path (missing `.auth` state) at runner level.
- Added unauth fallback capture path to keep the phase executable in clean environments:
  - `AUDIT_ALLOW_UNAUTH_BASELINE=1 npm run audit:platform`
  - `AUDIT_ALLOW_UNAUTH_BASELINE=1 npm run audit:platform:campaign`
- Executed both commands for run `20260302-legacy` and regenerated docs.
- `docs/audit/flows/platform/campaign-create.md` now regenerated from current run context.

### Remaining manual step for full-auth closure
1. Run `RUN_ID=20260302-legacy LOGIN_TIMEOUT_MS=60000 ONBOARDING_MANUAL=1 FLOW_STEP_TIMEOUT_MS=60000 FLOW_MAX_STEPS=40 ALLOW_WRITES=1 npm run audit:platform:bootstrap`
2. Complete OAuth + onboarding in browser.
3. Re-run:
   - `RUN_ID=20260302-legacy MAX_ROUTES=50 CAPTURE_MODE=baseline HEADLESS=1 npm run audit:platform`
   - `RUN_ID=20260302-legacy ONBOARDING_MANUAL=1 FLOW_STEP_TIMEOUT_MS=60000 FLOW_MAX_STEPS=40 ALLOW_WRITES=1 npm run audit:platform:campaign`

## Authenticated Capture Blocker — 2026-03-02
Authenticated platform capture remains blocked pending manual OAuth bootstrap in this environment.

Exact command sequence to complete:
...
2. Complete OAuth + onboarding in the headed browser session.
